...
export class JournalService {
  ...
  baserUrl="https://cynical-straw.up.railway.app"
  ...
  retrieveJournalList(currUser:string):Promise<any>{
    ...
    return lastValueFrom(
      this.http.get(`${this.baserUrl}/api/journal`, {
        params:params, 
  ...
  saveJournalEntry(journalEntry:journal):Promise<any>{
    ...
    return lastValueFrom(this.http.post(`${this.baserUrl}/api/saveEntry`,journalEntry, httpOptions))
  }
  
  getQuoteFromAPI():Promise<any>{
    return lastValueFrom(this.http.get(`${this.baserUrl}/api/QuoteApi`,
    {headers: this.getHttpOptions().headers}))
  }
  deleteEntry(date:string,user:string):Promise<any>{
    ...
    return lastValueFrom(this.http.put(`${this.baserUrl}/api/deleteEntry?user=${encodeURIComponent(user)}&date=${encodeURIComponent(date)}`, null, httpOptions));
  }
  undoDelete(id:string){
    ...
    const params = new HttpParams()
    .set("id", id)
    return lastValueFrom(this.http.post(`${this.baserUrl}/api/undo`,id,httpOptions))

  }
  sendNotification(title: string, message: string, token: string):Promise<any>{
    const data = { title, message, token };
    return lastValueFrom(this.http.post(`${this.baserUrl}/api/sendNotification`,data,this.getHttpOptions()))
  }
}
